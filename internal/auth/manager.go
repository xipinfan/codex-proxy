...
func (m *Manager) PickSessionAccount(sessionKey, model string, excluded map[string]bool) (*Account, error) {
	...
	if accountKey, ok := m.sessionAffinity.Lookup(sessionKey, now); ok {
		log.Infof("session affinity hit session=%q model=%s bound_account=%q excluded=%v", sessionKey, model, accountKey, excluded[accountKey])
		accounts := *m.accountsPtr.Load()
		for _, acc := range accounts {
			...
			if excluded != nil && excluded[acc.FilePath] {
				log.Infof("session affinity skip excluded session=%q account=%s", sessionKey, acc.GetEmail())
				return m.PickExcluding(model, excluded)
			}
			if accountPickableAt(now.UnixMilli(), model, acc) {
				log.Infof("session affinity reuse session=%q account=%s", sessionKey, acc.GetEmail())
				return acc, nil
			}
			log.Infof("session affinity evict unpickable session=%q account=%s", sessionKey, acc.GetEmail())
			m.sessionAffinity.EvictIfBound(sessionKey, accountKey)
			break
		}
	}
	if strings.TrimSpace(sessionKey) != "" {
		log.Infof("session affinity miss session=%q model=%s", sessionKey, model)
	}
	return m.PickExcluding(model, excluded)
}
...
func (m *Manager) BindResponseContinuation(responseID, sessionKey string, acc *Account) {
	...
	log.Infof("response continuation bind response_id=%q session=%q account=%s", responseID, sessionKey, acc.GetEmail())
}
...
func (m *Manager) ResolveSessionKeyFromResponseID(responseID string) string {
	...
	if !ok {
		if strings.TrimSpace(responseID) != "" {
			log.Infof("response continuation miss response_id=%q", responseID)
		}
		return ""
	}
	m.sessionAffinity.Bind(entry.sessionKey, entry.accountKey, time.Now())
	log.Infof("response continuation hit response_id=%q session=%q account_key=%q", responseID, entry.sessionKey, entry.accountKey)
